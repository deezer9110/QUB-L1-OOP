...
/**
 * Class allows the user to interact with a 1027 console instead of the system
 * one, serving the same functionality with a better UI design than the normal
 * QUBKitchen
 * 
...
public class QUBMediaKitchen {
	...
	public static ArrayList<Ingredient> allIngredients = QUBKitchen.allIngredients;
	public static ArrayList<Recipe> allRecipes = QUBKitchen.allRecipes;
	public static ArrayList<DailyMenu> allDailyMenus = QUBKitchen.allDailyMenus;
	public static ArrayList<String> availableDays = QUBKitchen.availableDays;
	public static String[] ingredientOptions = QUBKitchen.ingredientOptions;
	public static String[] recipeOptions = QUBKitchen.recipeOptions;
	public static Scanner input = new Scanner(System.in); // Creates a static input to be used throughout the code
	static Console con = createConsole();
	static Console displayCon = createConsole2();

	/**
	 * Creates console for menus and user input
	 * 
	 * @return base console
	 */
	public static Console createConsole() {

		Console con = new Console(true); // Creates editable console
		con.setVisible(true); // Sets it visible for changes to be made
		con.setLocation(25, 25); // Sets location on screen
		con.setSize(900, 800); // Sets size
		con.setFont(new Font("Arial", Font.BOLD, 30)); // Sets text style and font
		con.setColour(Color.WHITE); // Sets text colour
		con.setBgColour(Color.black); // Sets background colour
		return con; // Returns con
	}

	/**
	 * Creates console for display purposes
	 * 
	 * @return display console
	 */
	public static Console createConsole2() {
		Console con = new Console(false); // Creates uneditable console
		con.setVisible(true); // Sets it visible for changes to be made
		con.setLocation(925, 25); // Sets location on screen
		con.setSize(400, 800); // Sets size
		con.setFont(new Font("Arial", Font.BOLD, 20)); // Sets text style and font
		con.setColour(Color.WHITE); // Sets text colour
		con.setBgColour(Color.black); // Sets background colour
		return con; // Returns con
	}
	...
	public static void main(String[] args) {
		...
		mainMenu.displayConsole(con); // Displays on console

		do {
			try {
				choice = mainMenu.getChoiceConsole(con); // Runs method to take in the user's choice and process it
				switch (choice) {
				case 1:
					manageIngredients(); // Opens the Manage Ingredients menu
					mainMenu.displayConsole(con);
					break;
				case 2:
	 ...
	 * using con.print in every other method, allowing all other methods to be
	 * implemented easily into the Console
	 * 
	...
	public static String selectString(String prompt) {
		try {
			...
			result = con.readLn(); // Result reads and uses the next line for its value
			return result;
		} catch (Exception e) {
			con.println("Enter a valid choice"); // If an error is found, it will prompt the user to try again
													// until it's valid
	...
	public static int selectInt(String prompt) {
		try {
			...
			String resultS = con.readLn();
			result = Integer.parseInt(resultS); // Reads and saves the next inputed integer
			return result;
		} catch (Exception e) {
			con.println("Invalid option picked! Ensure valid input!"); // Prompts user to redo the method if an
																		// error is caught
	...
	public static String displayItemDetails(String menu, int index) {
		...
		case "Ingredient":
			ImageIcon image = getImage(allIngredients.get(index));
			if (image != null) {
				displayCon.print(image);
				displayCon.println();
			}
	...
	public static void manageIngredients() {
		...
		con.clear();
		displayCon.clear(); // Clears consoles
		ingredientsMenu.displayConsole(con); // Displays ingredients menu
		do {
			allIngredients = sortIngredients(allIngredients); // Sorts ingredients into alphabetical order
			try {
				choice = ingredientsMenu.getChoiceConsole(con); // Gets user choice for menu
				con.clear();
				displayCon.clear();
				switch (choice) {
				case 1: // Add ingredient
					if (addItem("Ingredient")) { // Runs method to add an item which will run the method to add an
													// ingredient
						displayCon.println("Ingredient added successfully! "); // Success message
					} else {
						displayCon.println("Ingredient not added "); // Failure message
					}
					con.clear();
					ingredientsMenu.displayConsole(con); // Redisplays ingredients menu
					break;
					...
					if (allIngredients.size() > 0) { // Checks that ingredient size is over 0
						...
						if (!checkCancel(index + 1)) { // Checks cancel
							if (removeItem("Ingredient", index)) { // Removes item
								displayCon.println("Ingredient removed!"); // Confirmation message
							}
						}
					} else {
						displayCon.println("No ingredients to remove"); // Displays if allIngredients is empty
					}
					con.clear();
					ingredientsMenu.displayConsole(con);
					break;
				case 3: // Removes all ingredients
					if (allIngredients.size() > 0 && removeAllItems("Ingredient")) {
						...
						displayCon.println("Ingredients removed!"); // Confirmation message
					} else {
						displayCon.println("No ingredients to remove"); // Stated if there aren't ingredients to remove
					}
					con.clear();
					ingredientsMenu.displayConsole(con);
					break;
				case 4: // Display all ingredients
					if (allIngredients.size() > 0) { // Checks size of allIngredients
						displayCon.println(displayAllItems("Ingredient")); // Displays all ingredients
					} else {
						displayCon.println("No ingredients to display!"); // Displays if list is empty
					}
					con.clear();
					ingredientsMenu.displayConsole(con);
					break;
				case 5: // Display ingredient details
					if (allIngredients.size() > 0) { // Ensures list has items
						...
						if (!checkCancel(index + 1)) { // Checks cancel
							displayCon.clear();
							displayCon.println(displayItemDetails("Ingredient", index)); // Displays details of
							// ingredient
						}
					} else {
						displayCon.println("No ingredients to display!"); // Displays if list is empty
								...
								if (!checkCancel(search1)) { // Checks cancel
									...
									if (results.size() > 0) { // Makes sure it's over 0
										for (Ingredient i : results) {
											displayCon.print(getImage(i));
											displayCon.println("\n" + i); // Prints each item in the results array

										}
									} else {
										displayCon.println("No results meet the criteria"); // Empty results list result
									}
								}
								break;
							case 1: // Search by name
								...
								if (!checkCancel(search2)) {
									...
									if (results.size() > 0) {
										...
										displayCon.println("No results meet the criteria");
									}
								}
								break;
							case 2: // Search by food group
								...
								if (!checkCancel(search3)) {
									...
									if (results.size() > 0) {
										...
										con.println("No results meet the criteria");
									}
								}
								break;
							default:
				...
				case 7: // Update ingredient
					if (allIngredients.size() > 0 && updateIngredientChoice(allIngredients)) {
						...
						displayCon.println("Ingredient updated!"); // Confirmation message
					} else {
						displayCon.println("No ingredients to update!"); // Empty list message
					}
					con.clear();
					ingredientsMenu.displayConsole(con);
				default:
				...
				displayCon.println("Unexpected error: " + e + "\nRestarting menu");
				con.clear();
				ingredientsMenu.displayConsole(con);
			}

			con.clear(); // Ensures base console is cleared and redisplays relevant menu
			ingredientsMenu.displayConsole(con);
		} while (choice != options.length); // Breaks loop if last (8th) option is picked

		displayCon.clear(); // Clears display console and returns to menu
		displayCon.print("Back to main menu: ");
		con.clear(); // Clears base console
	...
	public static boolean updateIngredientChoice(ArrayList<Ingredient> ingredientList) {
		try {
			...
			displayCon.clear();
			displayCon.println(allIngredients); // Display console prints all ingredients

			int index = 0;
			do {
				index = selectInt("Which ingredient would you like to update?") - 1; // Prompts for
																						// index
				if (checkCancel(index + 1)) { // Checks cancel
					return false;
				}
			...
			do {
				choice = selectInt("Which ingredient attribute do you want to update?: ");
	...
	public static void manageRecipes() {
		...
		int choice = 0;
		displayCon.clear(); // Clears display console
		recipesMenu.displayConsole(con); // Displays recipe menu
		do {
			allRecipes = sortRecipes(allRecipes); // Sorts recipes into alphabetical order
			try {
				choice = recipesMenu.getChoiceConsole(con); // Gets user choice for menu
				con.clear();
				displayCon.clear();
				switch (choice) {
				case 1: // Add recipe
					if (addItem("Recipe")) { // Runs method to add an item which will run the method to add an
												// recipe
						displayCon.println("Recipe added successfully! "); // Success message
					} else {
						displayCon.println("Recipe not added "); // Failure message
					}
					con.clear();
					recipesMenu.displayConsole(con); // Redisplays recipes menu
					break;
					...
					if (allRecipes.size() > 0) { // Checks that recipe size is over 0
						...
						if (!checkCancel(index + 1)) { // Checks cancel
							if (removeItem("Recipe", index)) { // Removes item
								displayCon.println("Recipe removed!"); // Confirmation message
							}
						}
					} else {
						displayCon.println("No recipes to remove"); // Displays if allRecipes is empty
					}
					con.clear();
					recipesMenu.displayConsole(con);
					break;
				case 3: // Removes all recipes
					if (allRecipes.size() > 0 && removeAllItems("Recipe")) {
						...
						displayCon.println("Recipes removed!"); // Confirmation message
					} else {
						displayCon.println("No recipes to remove"); // Stated if there aren't recipes to remove
					}
					con.clear();
					recipesMenu.displayConsole(con);
					break;
				case 4: // Display all recipes
					if (allRecipes.size() > 0) { // Checks size of allRecipes
						displayCon.println(displayAllItems("Recipe")); // Displays all recipes
					} else {
						displayCon.println("No recipes to display!"); // Displays if list is empty
					}
					con.clear();
					recipesMenu.displayConsole(con);
					break;
				case 5: // Display recipe details
					if (allRecipes.size() > 0) { // Ensures list has items
						...
						if (!checkCancel(index + 1)) { // Checks cancel
							displayCon.clear();
							displayCon.println(displayItemDetails("Recipe", index)); // Displays details of
							// recipe
						}
					} else {
						displayCon.println("No recipes to display!"); // Displays if list is empty
				...
				case 7: // Update ingredient
					if (allRecipes.size() > 0 && updateRecipeChoice()) {
						...
						displayCon.println("Recipe updated!"); // Confirmation message
					} else {
						displayCon.println("No recipes to update!"); // Empty list message
					}
					recipesMenu.displayConsole(con);
				default:
					con.clear();
			...
			con.clear();
			recipesMenu.displayConsole(con); // Ensures console is cleared and redisplays
	...
	public static boolean updateRecipeChoice() {
		try {
			...
			displayCon.clear();
			displayCon.println(allRecipesString);
	...
	public static boolean updateRecipeIngredientQuantity(int recipeIndex) {
		try {
			...
			displayCon.clear();
			displayCon.print(allIngredients);
	...
	public static boolean removeRecipeIngredient(int recipeIndex) {
		try {
			...
			int index = selectInt("Select an ingredient to remove: ") - 1;
	...
	public static void manageWeeklyMenu() {
		...
		int choice = 0;
		con.clear();
		weeklyMenu.displayConsole(con); // Displays menu
		do {
			try {
				...
				choice = weeklyMenu.getChoiceConsole(con); // Runs method to take in the user's choice and process it
				con.clear();
				displayCon.clear();
				switch (choice) {
				case 1: // Add daily menu
					if (allDailyMenus.size() < 5) {
						if (addItem("Daily Menu")) { // Runs method to add an item which will run the method to add an
														// daily menu
							displayCon.println("Daily Menu added successfully! "); // Success message
						} else {
							displayCon.println("Daily Menu not added "); // Failure message
						}
					} else {
						displayCon.println("No room for anymore daily menus");
					}
					con.clear();
					weeklyMenu.displayConsole(con); // Redisplays weekly menu
					break;
					...
					if (allDailyMenus.size() > 0) { // Checks that daily menu size is over 0
						...
						if (!checkCancel(index + 1)) { // Checks cancel
							if (removeItem("Daily Menu", index)) { // Removes item
								displayCon.println("Daily Menu removed!"); // Confirmation message
							}
						}
					} else {
						displayCon.println("No daily menus to remove"); // Displays if allDailyMenus is empty
					}
					con.clear();
					weeklyMenu.displayConsole(con);
					break;
				case 3: // Removes all daily menus
					if (allDailyMenus.size() > 0 && removeAllItems("Daily Menu")) {
						...
						displayCon.println("Daily menus removed!"); // Confirmation message
					} else {
						displayCon.println("No daily menus to remove"); // Stated if there aren't daily menus to remove
					}
					con.clear();
					weeklyMenu.displayConsole(con);
					break;
				case 4: // Display all daily menus
					if (allDailyMenus.size() > 0) { // Checks size of allDailyMenus
						displayCon.println(displayAllItems("Daily Menu")); // Displays all daily menus
					} else {
						displayCon.println("No daily menus to display!"); // Displays if list is empty
					}
					con.clear();
					weeklyMenu.displayConsole(con);
					break;
				case 5: // Display daily menu details
					if (allDailyMenus.size() > 0) { // Ensures list has items
						...
						if (!checkCancel(index + 1)) { // Checks cancel
							displayCon.clear();
							displayCon.println(displayItemDetails("Daily Menu", index)); // Displays details of
							// daily menu
						}
					} else {
						con.println("No daily menus to display!"); // Displays if list is empty
					}
					con.clear();
					weeklyMenu.displayConsole(con);
					break;
				case 6:
					if (allDailyMenus.size() > 0 && updateDailyMenuChoice()) { // Ensures daily menu sizes are over 0
						...
						displayCon.println("Daily menu updated!");
					} else {
						displayCon.println("No daily menus to update!");
					}
					con.clear();
					weeklyMenu.displayConsole(con);
				default:
	...
	public static boolean updateDailyMenuChoice() {
		...
		displayCon.clear();
		displayCon.println(allDailyMenusString); // Displays all daily menus
		...
		} while (choice < 1 || choice > 2); // Ensures choice is limited to current options
	...
	public static ImageIcon getImage(Ingredient i) {
		switch (i.getName()) {
		case "Apples":
			return new ImageIcon("Images\\Apples.png");
		case "BakedBeans":
			return new ImageIcon("Images\\BakedBeans.png");
		case "BellPeppers":
			return new ImageIcon("Images\\BellPeppers.png");
		case "Butter":
			return new ImageIcon("Images\\Butter.png");
		case "Celery":
			return new ImageIcon("Images\\Celery.png");
		case "Cheese":
			return new ImageIcon("Images\\Cheese.png");
		case "Eggs":
			return new ImageIcon("Images\\Eggs.png");
		case "Flour":
			return new ImageIcon("Images\\Flour.png");
		case "Garlic":
			return new ImageIcon("Images\\Garlic.png");
		case "GroundBeef":
			return new ImageIcon("Images\\GroundBeef.png");
		case "Jam":
			return new ImageIcon("Images\\Jam.png");
		case "Leeks":
			return new ImageIcon("Images\\Leeks.png");
		case "Lettuce":
			return new ImageIcon("Images\\Lettuce.png");
		case "Milk":
			return new ImageIcon("Images\\Milk.png");
		case "OliveOil":
			return new ImageIcon("Images\\OliveOil.png");
		case "Onions":
			return new ImageIcon("Images\\Onions.png");
		case "PennePasta":
			return new ImageIcon("Images\\PennePasta.png");
		case "Pepper":
			return new ImageIcon("Images\\Pepper.png");
		case "Potatoes":
			return new ImageIcon("Images\\Potatoes.png");
		case "Salt":
			return new ImageIcon("Images\\Salt.png");
		case "StockCubes":
			return new ImageIcon("Images\\StockCubes.png");
		case "Sugar":
			return new ImageIcon("Images\\Sugar.png");
		case "Tomatoes":
			return new ImageIcon("Images\\Tomatoes.png");
		case "Water":
			return new ImageIcon("Images\\Water.png");
		case "WhiteBread":
			return new ImageIcon("Images\\WhiteBread.png");
		default:
